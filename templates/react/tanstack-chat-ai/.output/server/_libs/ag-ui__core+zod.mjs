...
ZodPipeline.create;
//#endregion
//#region node_modules/.pnpm/@ag-ui+core@0.0.52/node_modules/@ag-ui/core/dist/index.mjs
...
var ToolCallSchema = objectType({
	...
	encryptedValue: stringType().optional()
});
...
var TextInputContentSchema = objectType({
	type: literalType("text"),
	text: stringType()
});
var InputContentSourceSchema = discriminatedUnionType("type", [objectType({
...
var ImageInputContentSchema = objectType({
	type: literalType("image"),
	source: InputContentSourceSchema,
	metadata: unknownType().optional()
});
var AudioInputContentSchema = objectType({
	type: literalType("audio"),
	source: InputContentSourceSchema,
	metadata: unknownType().optional()
});
var VideoInputContentSchema = objectType({
	type: literalType("video"),
	source: InputContentSourceSchema,
	metadata: unknownType().optional()
});
var DocumentInputContentSchema = objectType({
	type: literalType("document"),
	source: InputContentSourceSchema,
	metadata: unknownType().optional()
});
var LegacyBinaryInputContentObjectSchema = objectType({
...
LegacyBinaryInputContentObjectSchema.superRefine((value, ctx) => {
	ensureBinaryPayload(value, ctx);
});
var InputContentSchema = discriminatedUnionType("type", [
...
var MessageSchema = discriminatedUnionType("role", [
	...
	BaseMessageSchema.extend({
		...
		content: unionType([stringType(), arrayType(InputContentSchema)])
	}),
	objectType({
		id: stringType(),
		content: stringType(),
	...
	objectType({
		id: stringType(),
		role: literalType("activity"),
		activityType: stringType(),
		content: recordType(anyType())
	}),
	objectType({
		id: stringType(),
		role: literalType("reasoning"),
		content: stringType(),
		encryptedValue: stringType().optional()
	})
]);
unionType([
	...
	literalType("user"),
	literalType("tool"),
	literalType("activity"),
	literalType("reasoning")
]);
...
var RunAgentInputSchema = objectType({
	threadId: stringType(),
	runId: stringType(),
	parentRunId: stringType().optional(),
	state: anyType(),
...
var SubAgentInfoSchema = objectType({
	name: stringType(),
...
*/
var IdentityCapabilitiesSchema = objectType({
	name: stringType().optional(),
	type: stringType().optional(),
	description: stringType().optional(),
	version: stringType().optional(),
	provider: stringType().optional(),
	documentationUrl: stringType().optional(),
...
*/
var TransportCapabilitiesSchema = objectType({
	streaming: booleanType().optional(),
	websocket: booleanType().optional(),
	httpBinary: booleanType().optional(),
	pushNotifications: booleanType().optional(),
...
*/
var ToolsCapabilitiesSchema = objectType({
	supported: booleanType().optional(),
	items: arrayType(ToolSchema).optional(),
	parallelCalls: booleanType().optional(),
	clientProvided: booleanType().optional()
});
/**
...
*/
var OutputCapabilitiesSchema = objectType({
	structuredOutput: booleanType().optional(),
	supportedMimeTypes: arrayType(stringType()).optional()
});
/**
...
*/
var StateCapabilitiesSchema = objectType({
	snapshots: booleanType().optional(),
	deltas: booleanType().optional(),
	memory: booleanType().optional(),
	persistentState: booleanType().optional()
});
/**
* Multi-agent coordination capabilities. Enable these when your agent can
* orchestrate or hand off work to other agents.
*/
var MultiAgentCapabilitiesSchema = objectType({
	supported: booleanType().optional(),
	delegation: booleanType().optional(),
	handoffs: booleanType().optional(),
	subAgents: arrayType(SubAgentInfoSchema).optional()
});
/**
...
*/
var ReasoningCapabilitiesSchema = objectType({
	supported: booleanType().optional(),
	streaming: booleanType().optional(),
...
*/
var MultimodalInputCapabilitiesSchema = objectType({
	image: booleanType().optional(),
	audio: booleanType().optional(),
	video: booleanType().optional(),
	pdf: booleanType().optional(),
	file: booleanType().optional()
});
/**
* Modalities the agent can produce as output. Clients use this to anticipate
* rich content in the agent's response.
*/
var MultimodalOutputCapabilitiesSchema = objectType({
	image: booleanType().optional(),
	audio: booleanType().optional()
});
/**
...
*/
var MultimodalCapabilitiesSchema = objectType({
...
*/
var ExecutionCapabilitiesSchema = objectType({
	codeExecution: booleanType().optional(),
	sandboxed: booleanType().optional(),
	maxIterations: numberType().optional(),
	maxExecutionTime: numberType().optional()
});
/**
...
*/
var HumanInTheLoopCapabilitiesSchema = objectType({
	supported: booleanType().optional(),
	approvals: booleanType().optional(),
	interventions: booleanType().optional(),
	feedback: booleanType().optional()
});
objectType({
	...
	custom: recordType(unknownType()).optional()
});
var TextMessageRoleSchema = unionType([
	literalType("developer"),
	literalType("system"),
	literalType("assistant"),
	literalType("user")
...
var TextMessageStartEventSchema = BaseEventSchema.extend({
	type: literalType(EventType.TEXT_MESSAGE_START),
	messageId: stringType(),
	role: TextMessageRoleSchema.default("assistant"),
	name: stringType().optional()
});
var TextMessageContentEventSchema = BaseEventSchema.extend({
	type: literalType(EventType.TEXT_MESSAGE_CONTENT),
	messageId: stringType(),
	delta: stringType()
});
var TextMessageEndEventSchema = BaseEventSchema.extend({
	type: literalType(EventType.TEXT_MESSAGE_END),
	messageId: stringType()
});
var TextMessageChunkEventSchema = BaseEventSchema.extend({
	type: literalType(EventType.TEXT_MESSAGE_CHUNK),
...
*/
var ThinkingTextMessageStartEventSchema = BaseEventSchema.extend({ type: literalType(EventType.THINKING_TEXT_MESSAGE_START) });
/**
...
var RunFinishedEventSchema = BaseEventSchema.extend({
	type: literalType(EventType.RUN_FINISHED),
	threadId: stringType(),
	runId: stringType(),
	result: anyType().optional()
});
var RunErrorEventSchema = BaseEventSchema.extend({
	type: literalType(EventType.RUN_ERROR),
	message: stringType(),
	code: stringType().optional()
});
var StepStartedEventSchema = BaseEventSchema.extend({
	type: literalType(EventType.STEP_STARTED),
	stepName: stringType()
});
var StepFinishedEventSchema = BaseEventSchema.extend({
	type: literalType(EventType.STEP_FINISHED),
	stepName: stringType()
});
var ReasoningEncryptedValueSubtypeSchema = unionType([literalType("tool-call"), literalType("message")]);
discriminatedUnionType("type", [
	...
	BaseEventSchema.extend({
		type: literalType(EventType.REASONING_START),
		messageId: stringType()
	}),
	BaseEventSchema.extend({
		type: literalType(EventType.REASONING_MESSAGE_START),
		messageId: stringType(),
		role: literalType("reasoning")
	}),
	BaseEventSchema.extend({
		type: literalType(EventType.REASONING_MESSAGE_CONTENT),
		messageId: stringType(),
		delta: stringType()
	}),
	BaseEventSchema.extend({
		type: literalType(EventType.REASONING_MESSAGE_END),
		messageId: stringType()
	}),
	BaseEventSchema.extend({
		type: literalType(EventType.REASONING_MESSAGE_CHUNK),
		messageId: stringType().optional(),
		delta: stringType().optional()
	}),
	BaseEventSchema.extend({
		type: literalType(EventType.REASONING_END),
		messageId: stringType()
	}),
	BaseEventSchema.extend({
		type: literalType(EventType.REASONING_ENCRYPTED_VALUE),
		subtype: ReasoningEncryptedValueSubtypeSchema,
		entityId: stringType(),
		encryptedValue: stringType()
	})
]);
//#endregion
export { EventType as n, RunAgentInputSchema as r, AGUIError as t };
