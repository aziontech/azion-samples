...
import { Message, StreamEvent, TextMessage } from "../types";
/**
...
export class EdgeSQLTracerService {
   ...
   * @param {string[]} [azionMetadata] - Array of metadata messages from the agent
   */
  constructor(
    mode: 'stream' | 'invoke',
    databaseName: string,
    ...
    this.retryCount = 0
    this.azionMetadata = JSON.stringify(azionMetadata)
  }
  ...
  createInsertStatement(): string[] {
    ...
    const statements = [`INSERT INTO ${this.tableName} (session_id, run_id, input_messages, output_messages, run_metadata, azion_metadata, created_at) VALUES ('${this.sessionId}', '${this.runId}', '${this.inputMessages}', '${this.outputMessages}', '${this.runMetadata}', '${this.azionMetadata}', '${createdAt}')`]
    return statements
  }
  ...
  async save() {
    ...
    const { data, error } = await useExecute(this.databaseName, this.createInsertStatement(), { debug: true })
