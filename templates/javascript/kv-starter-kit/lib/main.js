...
export const mainHTML = (userIP) => {
  ...
  <head>
      ...
      <title>KV Starter Kit</title>
      <meta name="description" content="KV Starter Kit" />
      ...
      <style>
        *, *::before, *::after {
        ...
        a:hover {
            background: rgba(180, 185, 188, 0.15);
        ...
        .content::after {
            ...
            background: conic-gradient(from 180deg at 50% 50%, #502e2233 0deg, #502e2233 55deg, #502e2233 120deg, #502e2233 160deg, transparent 360deg);
            width: 240px;
            height: 180px;
            z-index: -1;
        }
        ...
        .content-info .instructions h4 {
            font-size: 14pt;
            text-align: center;
        }
        .content-info .instructions p {
            font-size: 12pt;
            opacity: 0.8;
            max-width: 100%;
            line-height: 3;
        }
        .content-info .instructions a, .content-info .instructions code {
            color:#fdba74;
        }
        ol {
            padding: 0;
        ...
        .list-content div.actions {
            display: flex;
        }
        div.control button, div.control input {
            padding: 10px 12px;
  ...
  <body>
          ...
          <div class="content">
              ...
              <div class="content-info">
                  <div class="instructions">
                      <h1>KV Starter Kit</h1>
                      <p>
                        This page showcases how to use KV to store and retrieve data.
                      </p>
                      <p> 
                        In this example, we use a key to store an IP and block requests made from it.
                      </p>
                      <p>
                        Add your IP to the block list using the form below, then go to <a href="./am-i-blocked" id="blocklink">/am-i-blocked</a> under the IP and it'll be blocked. Your IP is: <code>${userIP}</code>.
                      </p>
                  </div>
                  <div class="control">
                    <div>
                        <input type="text" id="block-ip" name="block-ip" value="${userIP}" onkeydown="handleBlockEnterKey(event)">
                        <button id="block-ip" onclick="blockIp()">
                            Block IP
                        </button>
                    </div>
                    <div>
                        <input type="text" id="unblock-ip" name="unblock-ip" value="${userIP}" onkeydown="handleUnblockEnterKey(event)">
                        <button id="unblock-ip" onclick="unblockIp()">
                            Unblock IP
                        </button>
      ...
      <script type="application/javascript" defer>
        function handleBlockEnterKey(event) {
            if (event.keyCode === 13 || event.key === 'Enter') {
                blockIp();
            }
        }
        function handleUnblockEnterKey(event) {
            if (event.keyCode === 13 || event.key === 'Enter') {
                unblockIp();
            }
        }
        ...
        async function blockIp() {
            ...
            let response = await fetch("/block-ip", {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                alert("Failed to block the IP! Check the Azion Real-Time Events for more details.");
                return;
            }

            alert("The IP will be blocked for five minutes!");
        }
        ...
        async function unblockIp() {
            ...
            let response = await fetch("/unblock-ip", {
                method: 'POST',
                headers: {
                    "Accept": "application/json",
                    "Content-Type": "application/json"
            ...
            if (!response.ok) {
                alert("Failed to unblock the IP! Check the Azion Real-Time Events for more details.");
                return;
            }

            alert("The IP was removed from the block list!");
        }
      </script>
  </body>
