...
import { createDatabase, useExecute } from "@aziontech/sql";
...
export class EdgeSQLTracerService {
   ...
   */
  updateInput(messages: Message[], runId: string): void {
    this.inputMessages = messages.map((message: Message) =>
  ...
  async updateStreamOutput(
    ...
    for await (const streamEvent of eventStream) {
      ...
      if (
        ...
        streamEvent.tags?.includes("agent")
      ) {
    ...
    this.runMetadata = metadata
      .at(-1)
      .map((message: any) => JSON.stringify(message));
  }
   ...
   */
  updateInvokeOutput(messages: Message[]): void {
   ...
   */
  async run(outputMessages: any | ReadableStream<StreamEvent>): Promise<void> {
    try {
      if (this.mode === "stream") {
  ...
  removeSingleQuotesFromStrings(): void {
    this.inputMessages = this.inputMessages.map((message: string) =>
      message.replace(/'/g, ""),
    );
    this.outputMessages = this.outputMessages.replace(/'/g, "");
    this.runMetadata = this.runMetadata?.map((message: string) =>
      message.replace(/'/g, ""),
    );
  }
  ...
  createInsertStatement(): string[] {
    ...
    const statements = [
      `INSERT INTO ${this.tableName} (session_id, run_id, input_messages, output_messages, run_metadata, created_at) VALUES ('${this.sessionId}', '${this.runId}', '${this.inputMessages}', '${this.outputMessages}', '${this.runMetadata}', '${createdAt}')`,
    ];
    return statements;
  }
  ...
  createSetupTableStatement(): string[] {
    const statements = [
      `CREATE TABLE IF NOT EXISTS ${this.tableName} (id INTEGER PRIMARY KEY AUTOINCREMENT, session_id TEXT, run_id TEXT, input_messages TEXT, output_messages TEXT, run_metadata TEXT, created_at TEXT)`,
    ];
    return statements;
  }
  ...
  async save() {
    ...
    const { data, error } = await useExecute(
      this.databaseName,
  ...
  async handleErrors(error: any) {
    ...
    if (error.message.includes("no such table")) {
      ...
      const { error: createTableError } = await useExecute(
        this.databaseName,
        this.createSetupTableStatement(),
      );
      if (createTableError) {
